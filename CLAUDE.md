...
A brand-new `internal/client/**/*.go` file starts every PR at a coverage disadvantage against that ratchet — it's compared against `internal/client`'s mature, multi-PR-averaged coverage before it's had a chance to accumulate the same depth of tests older files have (#204's `nna_source_group.go` at 60%, #213's `nna_user.go` at 66-67%, both flagged by `codecov/patch/client` despite genuine, non-filler coverage). Codecov's own config has no "this file is new to this PR" concept to carve that out, so `.github/workflows/test.yml`'s `test` job computes it itself: any `*.go` file added (not modified) under `internal/client/` since the PR's base commit is held to a fixed 50% floor instead of the ratchet, then stripped out of `coverage.out` before upload so Codecov's ratchet math never sees it at all for that PR. Once merged, the file exists in the next PR's base commit and stops being "new" — it's ratcheted normally like every other existing file from then on, with no follow-up cleanup step required (#205).

...
12. **`get()` doesn't route through `parseCommandResponse`** the way POST/PUT/DELETE do (see quirk 1) — in principle an error response could be indistinguishable from a genuine zero-result GET. Confirmed against a live instance (#89): an auth failure (bad/missing API key) returns a JSON *object* (`{"error":"..."}`), never an empty array, and `json.Unmarshal` into a `[]X` slice already fails loudly on that shape rather than silently decoding to zero results — so `GetX`'s `len(results)==0` check can't confuse the two for this verified case. Not a blanket guarantee for every conceivable error/permission-scoping response shape, only the one actually tested (see `TestGetHost_ErrorObjectResponseIsNotSilentlyTreatedAsNotFound`). This protection is specific to the bare-`[]X`-array response shape: `authserver` and `user` both decode into a `{"records", "<type>":[...]}` envelope struct instead (quirks 6 and 14), and an error object's stray keys are silently ignored by `json.Unmarshal` rather than causing a decode failure — `GetAuthServer`/`GetUser` have no equivalent guarding test yet, an open gap rather than a verified-safe case.
